...
class Fuzz
{
    ...
    public function search(string $search, int $minLCS = null, int $maxDistance = null)
    {
        ...
        for ($i = 0; $i < $this->_sourceLen; $i++) {
            ...
            foreach ($this->_source[$i] as $key => $val) {
        ...
        // Retrieve Objs from source pool
        $resultLen = min($this->_maxResults, count($scores));
        for ($i = 0; $i < $resultLen; $i++) {
            $results[] = $this->_source[$scores[$i]['index']];
        }
